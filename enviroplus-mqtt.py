...
from ha_mqtt_device import Sensor, mqtt
...
def create_device(config, device_serial_number):
    ...
    mqtt_client = mqtt.Client(client_id=f"raspi-{device_serial_number}")
    ...
    device = {}
    device["identifiers"] = [device_serial_number]
    device["manufacturer"] = config.get("device", "manufacturer")
    device["model"] = config.get("device", "model")
    device["name"] = config.get("device", "name")

    mqtt_client.connect(broker, port=port)

    return mqtt_client, device


def create_sensor(mqtt_client, config, device):
    ...
    Sensor(
        ...
        value_template="reduced",
        icon="mdi:thought-bubble",
    )

    Sensor(
        client=mqtt_client,
        friendly_name="Oxidised",
        parent_device=device,
        unit_of_measurement="ppm",
        state_topic=topic,
        device_class="nitrogen_dioxide",
        value_template="oxidised",
        icon="mdi:thought-bubble",
    )


def run_mqtt(mqtt_client, config, device_serial_number):
    broker = config.get("mqtt", "broker_ip")
    topic = config.get("mqtt", "topic")
...
def main():
    ...
    mqtt_client, device = create_device(config, device_serial_number)
    print(device)
    create_sensor(mqtt_client, config, device)
    run_mqtt(mqtt_client, config, device_serial_number)
